...
void MacTdma::send() 
{
	...
	// std::cout << pktTx_->time_ << std::endl;
    pktTx_ = 0;
}
...
{
	DirAntenna** dir_antennas = Simulator::instance().dir_antennas;
	...
	if (slot_count_ == slot_num_) {//到自己时隙
		int width = dir_antennas[slot_num_]->getwidth();
		int original_angle = dir_antennas[slot_count_]->getangle();
		 if (slot_count_ == 0) {//判断是否为0节点
			if (tdma_preamble_[slot_num_] != NOTHING_TO_SEND) {
				send();
				dir_antennas[slot_num_]->setangle(original_angle + 360/32);//改变0节点的角度
				turn0_count_++;
				if(turn0_count_==32){//节点0遍历一周
					turn0_count_=0;
					std::cout << "one round time:" << NOW << std::endl;
				}
		...
		// 	}
			...
			if (turn0_count_ == 0) {
				dir_antennas[slot_num_]->setangle(original_angle + 360/32);//改变当前节点的角度
			}
			if (slot_count_ == 11) {
				std::cout << dir_antennas[slot_num_]->getangle() << endl;
			}
		}
